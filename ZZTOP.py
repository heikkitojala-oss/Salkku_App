...
EXCHANGE_RATES_FILE = "exchange_rates.json" # Uusi tiedostonimi
        ...
        if os.path.getsize(HISTORY_FILE) > 0:
            with open(HISTORY_FILE, 'r') as f:
                try:
                    ...
                    st.warning("Varoitus: portfolio_history.json-tiedosto on vioittunut. Luodaan uusi.")
                    return {}
        else:
            return {}
    return {}
...
def load_exchange_rates():
    """Lataa valuuttakurssit tiedostosta tai asettaa oletusarvot."""
    if os.path.exists(EXCHANGE_RATES_FILE):
        with open(EXCHANGE_RATES_FILE, 'r') as f:
            return json.load(f)
    # Oletusarvot, jos tiedostoa ei ole
    return {
        "EUR": 1.0,
        "USD": 1.07,  # Esimerkki
        "SEK": 11.5,
...
def save_exchange_rates(rates):
    """Tallentaa valuuttakurssit tiedostoon."""
...
def calculate_portfolio_metrics(assets, current_prices, exchange_rates):
    ...
    for asset in assets:
        ...
        # TÄMÄ ON UUSI RIVI, JOKA KÄYTTÄÄ TALLENNETTUJA VALUUTTOJA
        current_currency_rate = exchange_rates.get(currency, 1.0)
...
def display_portfolio_summary(df, total_row, portfolio_name):
    ...
    if portfolio_name in history and len(history[portfolio_name]) > 1:
        ...
        bar_chart = alt.Chart(history_df).mark_bar().encode(
...
def main():
    ...
    # --- Uusi valuuttakurssien hallinta sivupalkissa ---
    st.sidebar.markdown("---")
    st.sidebar.subheader("Valuuttakurssit (1 EUR = X valuuttaa)")
    exchange_rates = load_exchange_rates()
    
    # Valuuttalistaa voidaan nyt muokata täällä
    currencies = ["EUR", "USD", "SEK", "GBP", "AUD", "NOK", "DKK", "JPY", "CAD"]
    
    for currency in currencies:
        if currency == "EUR":
            st.sidebar.number_input(f"EUR:", value=1.0, disabled=True, key="eur_rate")
        else:
            exchange_rates[currency] = st.sidebar.number_input(
                f"{currency}:", 
                value=float(exchange_rates.get(currency, 1.0)), 
                min_value=0.0001,
                step=0.001,
                format="%.4f",
                key=f"{currency}_rate"
            )
    
    if st.sidebar.button("Tallenna valuuttakurssit"):
        save_exchange_rates(exchange_rates)
        st.sidebar.success("Valuuttakurssit tallennettu!")
        st.rerun()
    # --- Valuuttakurssien hallinta loppuu ---
    
    # --- Uusi kokonaissalkun tallennusnappi sivupalkissa ---
    st.sidebar.markdown("---")
    st.sidebar.subheader("Kokonaisarvon tallennus")
    if st.sidebar.button("Tallenna kaikkien salkkujen arvo", key="save_all_button"):
        st.write("Haetaan hintatiedot ja tallennetaan...")
        history = load_portfolio_history()
        
        all_tickers = []
        for assets in portfolios.values():
            all_tickers.extend([asset['ticker'] for asset in assets if not asset.get('is_manual') and asset.get('ticker')])
        
        unique_tickers = list(set(all_tickers))
        current_prices = get_stock_data(unique_tickers)
        
        today = date.today().isoformat()
        
        for portfolio_name, assets in portfolios.items():
            # TÄMÄ RIVI ON KORJATTU
            df, total_row = calculate_portfolio_metrics(assets, current_prices, exchange_rates)
            
            total_current_value = float(total_row["Nykyinen arvo"].iloc[0])
            
            if portfolio_name not in history:
                history[portfolio_name] = []
            
            existing_dates = [item[0] for item in history[portfolio_name]]
            if today in existing_dates:
                for item in history[portfolio_name]:
                    if item[0] == today:
                        item[1] = total_current_value
            else:
                history[portfolio_name].append([today, total_current_value])
                
        save_portfolio_history(history)
        st.success("Kaikkien salkkujen arvo tallennettu onnistuneesti!")
        st.rerun()
    # --- Loppu uusi koodi ---
    ...
    for i in range(1, int(num_assets) + 1):
        ...
        with col1:
            ...
            if manual_input:
                ...
                currency = st.selectbox("Valuutta:", currencies, key=f"manual_currency_{i}", index=currencies.index(asset_data.get('currency', 'EUR')))
            else:
                manual_price = None
                currency = st.selectbox("Valuutta:", currencies, key=f"currency_{i}", index=currencies.index(asset_data.get('currency', 'EUR')))
        ...
        with col2:
            ...
            # Nykykurssia ei enää syötetä tässä, vaan se haetaan tallennetuista arvoista
            st.markdown(f"**Nykykurssi (1 EUR = X {currency}):** `{exchange_rates.get(currency, 1.0):.4f}`")
        ...
        assets.append({
            ...
            "current_currency_rate": exchange_rates.get(currency, 1.0), # Tallennetaan automaattisesti
    ...
    with tab1:
        ...
        if selected_portfolio_name != "Uusi salkku":
            if st.button(f"Tarkastele salkkua '{selected_portfolio_name}'"):
                ...
                # TÄMÄ RIVI ON KORJATTU
                df, total_row = calculate_portfolio_metrics(portfolios[selected_portfolio_name], current_prices, exchange_rates)
                ...
                if st.button("Tallenna salkun tämänhetkinen arvo", key="save_single_button"):
                    history = load_portfolio_history()
                    ...
                    if today in existing_dates:
                        ...
                        history[selected_portfolio_name].append([today, float(total_current_value)])
                        save_portfolio_history(history)
